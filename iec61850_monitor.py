...
class COMTradeGenerator:
    ...
    def generate_cff(self, output_path: str) -> bool:
        """Generate COMTRADE CFF (Common Format File)"""
        ...
        try:
            ...
            cfg_path = output_path.replace('.cff', '.cfg')
            if output_path.endswith('.cff'):
                cfg_path = output_path[:-4] + '.cfg'
            else:
            ...
            # Generate .dat file
            dat_path = output_path.replace('.cff', '.dat')
            if output_path.endswith('.cff'):
                dat_path = output_path[:-4] + '.dat'
            else:
            ...
            print(f"Generated COMTRADE files: {cfg_path}, {dat_path}")
            return True
            
        except Exception as e:
            print(f"Error generating COMTRADE: {e}")
            return False
    
    def _generate_cfg(self) -> str:
        """Generate COMTRADE configuration file content"""
        lines = []
        
        # Station name (line 1)
        lines.append(f"IEC 61850 Monitor - {self.gse_control.ied_name}")
        
        # Device ID (line 2)
        lines.append(f"GSE:{self.gse_control.name}")
        
        # Time code format (line 3) - D = absolute time in ASCII
        lines.append("1970,1,1,0,0,0,0")
        
        # Number of channels (line 4)
        num_analog = len(self.analog_channels)
        num_digital = len(self.digital_channels)
        lines.append(f"{num_analog},{num_digital}")
        
        # Analog channel definitions (lines 5 to 5+num_analog-1)
        for i, ch in enumerate(self.analog_channels):
            # Format: name, units, a,b,c,d,offset,min,max
            lines.append(f"{ch['name']},V,1,0,0,0,0,-32768,32767")
        
        # Digital channel definitions
        for i, ch in enumerate(self.digital_channels):
            # Format: name, units, a,b,c,d,offset,min,max
            lines.append(f"{ch['name']},Status,1,0,0,0,0,0,1")
        
        # Line frequency (next line)
        lines.append("50.0")
        
        # Number of samples per cycle (next line)
        if self.smv_control and self.smv_control.smp_rate > 0:
            smp_per_cycle = self.smv_control.smp_rate // 50
        else:
            smp_per_cycle = 80  # default
        lines.append(str(smp_per_cycle))
        
        # Data format (next line) - A = ASCII
        lines.append("A")
        
        # Time multiplier (next line)
        lines.append("1")
        
        # Time synchronization (next line) - 0 = not synchronized
        lines.append("0")
        
        # Trigger information (next lines)
        lines.append("")  # Trigger type
        lines.append("0")  # Trigger sample number
        lines.append("0")  # One second before trigger
        lines.append("0")  # One second after trigger
        lines.append("")  # Primary equipment rating
        lines.append("")  # Primary equipment rating
        lines.append("")  # CT/VT ratio
        lines.append("")  # CT/VT ratio
